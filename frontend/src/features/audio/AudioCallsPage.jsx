...
export const AudioCallsPage = () => {
  ...
  // Active call states
  const [activeCall, setActiveCall] = useState(null); // { creator, status: 'connecting' | 'ringing' | 'active', roomId, duration: 0 }
  ...
  const simulatedAcceptTimer = useRef(null);
  ...
  const loadCreators = () => {
    ...
    // Pill Availability Filter
    if (activePill === 'Online Now') {
      filtered = filtered.filter(c => c.isOnline && !c.isBusy);
    }

    // Sidebar Filters
    filtered = filtered.filter(c => c.voiceCallMinute <= priceRange);
    ...
    if (activePill === 'Popular') {
      ...
      filtered.sort((a, b) => a.voiceCallMinute - b.voiceCallMinute);
    } else if (activePill === 'Price: High to Low') {
      filtered.sort((a, b) => b.voiceCallMinute - a.voiceCallMinute);
    }
  ...
  const startCall = async (creator) => {
    ...
    if (balance < creator.voiceCallMinute) {
      alert(`Insufficient coins! You need at least ${creator.voiceCallMinute} coins to initiate this call.`);
      return;
    }

    try {
      const res = await api.post('/call/initiate', {
      ...
      if (res.status === 'success') {
        ...
        const callLogId = res.callLog._id;
        ...
        setActiveCall({
          ...
          roomId,
          callLogId,
          rate: creator.voiceCallMinute
        });
        setCallDuration(0);

        // Ringing state simulation
        simulatedAcceptTimer.current = setTimeout(() => {
          setActiveCall(prev => prev ? { ...prev, status: 'ringing' } : null);

          // Simulated acceptance after 3 seconds
          simulatedAcceptTimer.current = setTimeout(() => {
            setActiveCall(prev => prev ? { ...prev, status: 'active' } : null);
            refreshBalance();
            startDurationAndBilling(roomId);
          }, 3000);
        }, 1500);
  ...
  const startDurationAndBilling = (roomId) => {
    ...
    heartbeatTimer.current = setInterval(async () => {
      try {
        const hbRes = await api.post('/call/heartbeat', { roomId });
        if (hbRes.status === 'terminated') {
          alert('Call disconnected: Insufficient coins.');
          hangUp();
        } else {
  ...
  const hangUp = async () => {
    ...
    if (simulatedAcceptTimer.current) clearTimeout(simulatedAcceptTimer.current);

    if (activeCall && activeCall.roomId) {
      try {
        await api.post('/call/end', { roomId: activeCall.roomId });
      } catch (err) {
        console.error('Failed to end call session on backend:', err);
      }
    }
                      ...
                      <div className={styles.cardBody}>
                        ...
                        <div className={styles.statsRow}>
                          ...
                          <span className={styles.coinRateInfo}>
                            ...
                            {creator.voiceCallMinute} Coin/Min
                          </span>
                        </div>
