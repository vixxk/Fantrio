...
// Detailed stream analytics: tipping/gift logs, gifts leaderboard, viewers list
exports.getStreamDetails = catchAsync(async (req, res, next) => {
  ...
  // Creator can only view their own stream details, unless admin
  if (req.user.role === 'creator' && stream.creatorId && stream.creatorId._id.toString() !== req.user._id.toString()) {
    return next(new ApiError(403, 'You do not have permission to view details for this stream'));
  }

  const Transaction = require('../models/Transaction');

  // Fetch transactions associated with this stream
  const transactions = await Transaction.find({
    referenceId: streamId,
    status: 'completed'
  })
